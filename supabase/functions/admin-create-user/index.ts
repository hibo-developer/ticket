...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceKey)
    return json(corsOrigin, 500, { error: 'Falta configuración de Supabase (SUPABASE_URL, anon, service).' })
  ...
  if (meError || !me?.org_id) return json(corsOrigin, 403, { error: 'No autorizado (perfil no encontrado).' })
  if (!me.active) return json(corsOrigin, 403, { error: 'Cuenta desactivada.' })
  if (me.app_role !== 'admin') return json(corsOrigin, 403, { error: 'No autorizado (no admin).' })
  ...
  if (!validPassword(tempPassword))
    return json(corsOrigin, 400, { error: 'Contraseña temporal débil (mín. 10 chars, letras + números).' })
  if (username && !validUsername(username))
    return json(corsOrigin, 400, { error: 'Nombre de usuario inválido (3-32, letras/números/._-).' })
  ...
  if (roleIds.length > 0) {
    ...
    if (validRoleIds.length !== roleIds.length)
      return json(corsOrigin, 400, { error: 'Algún role_id no pertenece a esta organización.' })
  }
